...
document.addEventListener("DOMContentLoaded", function () {
    ...
    for (const group of formGroups) {
        ...
        const fetchData = async (query) => {
            ...
            const hiddenInputId = loaderWindow.dataset.hiddenInputId;
            
            if (hiddenInputId.startsWith('editors_pick')) {
                endpoint = URL.VIDEO_SEARCH.EDITORS_PICK;
            } else if (hiddenInputId.startsWith('new')) {
                endpoint = URL.VIDEO_SEARCH.NEW;
            } else if (hiddenInputId.startsWith('throwback')) {
                endpoint = URL.VIDEO_SEARCH.THROWBACK;
            } else {
                console.error(`No available endpoints for (${hiddenInputId})`);
            }
            try {
                const response = await fetch(`${URL.HOST}${endpoint}`, {
                    method: 'POST',
                    headers: {
